"""
run_s2_confluence.py — S2A × S2B 合流分析：合併假設檢驗
==============================================================================
輸出：xauusd/XAUUSD-Long-S2B-Hammer/report_s2_confluence.html

背景（20260711）：使用者考慮把 S2A-RSI 與 S2B-Hammer 合併（因單獨勝率 42-44% 偏低）。
合併若有意義，前提是「兩者同時觸發」的交易品質應高於單獨觸發——本腳本用兩份
真實逐筆交易 CSV 直接檢驗這個前提。

結論先講：**前提不成立，方向完全相反。**
S2B 錘頭在 ±12h 內「沒有」S2A 訊號時：WR 50.4%、PF 2.14（n=135）
S2B 錘頭與 S2A 同時觸發時：      WR 30.8%、PF 1.03（n=65，打平）
且時間切分驗證兩段皆成立，OOS 段合流組 PF 僅 0.57（虧損）。

機制解釋（與既有研究吻合）：S2A 觸發 = RSI 深度超賣/背離環境。
2026-07-02 的 Regime 四象限分析已發現 Z-Score < -2.5 極端超賣是陷阱區
（勝率僅 20%，極端超賣代表趨勢延伸而非反轉）。錘頭出現在 RSI 同時深度超賣
的時刻＝接刀接在瀑布中段；錘頭單獨出現才是健康回調的反轉點。

行動建議：不合併。改為 S2B 加「S2A 互斥過濾器」（S2A 環境成立 → S2B 跳過），
S2A 考慮降級為環境 veto 濾網（待 S2 真實逐筆歸因後決定）。
...
S2A_CSV  = ROOT / "xauusd/XAUUSD-Long-S2A-RSI/S2-Hybrid-V2.0_FX_IDC_XAUUSD_2026-04-26.csv"
S2B_CSV  = ROOT / "xauusd/XAUUSD-Long-S2B-Hammer/S2-Pullback-V1.9_FX_IDC_XAUUSD_2026-04-26.csv"
OUT_HTML = ROOT / "xauusd/XAUUSD-Long-S2B-Hammer/report_s2_confluence.html"

WINDOWS_H = [2, 6, 12]   # 合流判定窗口（小時）
MAIN_W_H  = 12           # 主結論採用的窗口

# ── 1. 載入 ──────────────────────────────────────────────────────────────────
print("載入 S2A / S2B 真實逐筆交易...")
a = loader.load_trades(S2A_CSV)
b = loader.load_trades(S2B_CSV)
print(f"  S2A: {len(a)} 筆（{a.entry_time.min().date()} → {a.entry_time.max().date()}）")
print(f"  S2B: {len(b)} 筆（{b.entry_time.min().date()} → {b.entry_time.max().date()}）")
...
for h in WINDOWS_H:
    ...
    rows.append(dict(window=h, group="S2B 單獨", **stat_block(b[~b_conf])))
    rows.append(dict(window=h, group="S2B 有S2A合流", **stat_block(b[b_conf])))
    rows.append(dict(window=h, group="S2A 單獨", **stat_block(a[~a_conf])))
    rows.append(dict(window=h, group="S2A 有S2B合流", **stat_block(a[a_conf])))
...
def chart_headline():
    ...
    a1.set_title(f"S2B 勝率 %（合流窗口 ±{MAIN_W_H}h）", fontsize=12)
    ...
    a2.set_title("S2B 獲利因子", fontsize=12)
...
def chart_split():
    ...
    ax.bar(x - w/2, alone, w, label="S2B 單獨", color="#22c55e")
    ax.bar(x + w/2, conf,  w, label="S2B 有S2A合流", color="#ef4444")
...
def split_table():
    ...
    for seg_name, label in [("IS", f"前70%（~{cut.date()}）"), ("OOS", f"後30%（{cut.date()}~）")]:
        ...
        body += (f"<tr><td rowspan='2'><strong>{label}</strong></td>"
                 f"<td style='color:#22c55e'>S2B 單獨</td><td>{d['alone']['n']}</td>"
                 f"<td>{fmt(d['alone']['wr'],'{:.1f}')}%</td><td>{fmt(d['alone']['pf'],'{:.3f}')}</td>"
                 f"<td>{fmt(d['alone']['avg'],'{:+.3f}')}%</td></tr>"
                 f"<tr><td style='color:#ef4444'>S2B 有S2A合流</td><td>{d['conf']['n']}</td>"
...
<title>S2A × S2B 合流分析：合併假設檢驗</title>{CSS}</head>
<body>
...
<div class="wrap">
<h1>S2A × S2B <span style="color:#f59e0b">合流分析</span> — 合併假設檢驗</h1>
<p class="note">生成時間：{pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')} ·
資料：S2A V2.0（{len(a)} 筆）+ S2B V1.9（{len(b)} 筆）真實逐筆交易，2024-01 ~ 2026-04</p>

<div class="bad">
<strong>結論：不要合併。合流是反指標。</strong>
原始假設「S2A（RSI）與 S2B（錘頭）同時觸發 = 更高品質訊號」被資料推翻——
S2B 錘頭在 ±{MAIN_W_H}h 內<strong>沒有</strong> S2A 訊號時 WR {s_alone_all['wr']:.1f}% / PF {s_alone_all['pf']:.2f}（n={s_alone_all['n']}），
與 S2A 同時觸發時僅 WR {s_conf_all['wr']:.1f}% / PF {s_conf_all['pf']:.2f}（n={s_conf_all['n']}，接近打平）。
</div>

<div class="card">
<div class="part">PART 1 — 主結果</div>
<h2>S2B 分組績效（合流窗口 ±{MAIN_W_H}h）</h2>
{img(img_headline)}
<p class="note">現行 S2B 全收：WR {s_base_all['wr']:.1f}% / PF {s_base_all['pf']:.2f}。
只要跳過「S2A 也在觸發」的錘頭訊號，剩餘 {s_alone_all['n']}/{s_base_all['n']} 筆的品質顯著提升。</p>
</div>
...
{sweep_table()}
<p class="note">綠底 = S2B 單獨、紅底 = S2B 合流。效果在 ±6h 起顯著且方向一致（±2h 樣本太少，n=21）。
S2A 自身的合流/單獨差異不大（42% 上下），資訊價值集中在「它標記的環境對 S2B 有害」。</p>
</div>
...
S2A 觸發 = RSI 深度超賣/背離環境。2026-07-02 的 Regime 四象限分析
...
<h2>取代「合併」的三個行動</h2>
<div class="good"><strong>1. S2B 加「S2A 互斥過濾器」</strong>：S2A 觸發環境（近 {MAIN_W_H}h 內 RSI 深超賣/背離條件成立）→ S2B 跳過。
預期效果：WR {s_base_all['wr']:.1f}%→{s_alone_all['wr']:.1f}%、PF {s_base_all['pf']:.2f}→{s_alone_all['pf']:.2f}，保留 {s_alone_all['n']}/{s_base_all['n']} 筆交易。</div>
<div class="good"><strong>2. S2A 重新定位</strong>：其資訊價值可能不在「何時進場」而在「何時不要進」——
待 S2 真實逐筆歸因（NEXT 優先事項）完成後，決定 S2A 是保留為獨立策略還是降級為 S2B 的 veto 濾網。</div>
...
S2A V2.4 / S2B V2.3 已實作的 Z-Score 過濾器（甜蜜區 -1.5~-0.5、阻擋 &lt;-2.5）是否為同一件事——
...
時間切分按 S2B 筆數 70/30。統計口徑與 xauusd/analysis/ 既有模組一致。
</p>
...
XAUUSD S2A × S2B 合流分析 · {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M')}
</footer>
</div></body></html>"""
