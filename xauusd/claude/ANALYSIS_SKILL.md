...
② 備份：讀 PNG（.gdoc 不存在或 Chrome 無法操作時）
  路徑：/Users/tittan/googledrive/XAUUSD/weekly report/png/*.png
  找最新的 XAUUSD_Weekly_Report_*.png，用 Read 工具讀圖（Claude 可視覺解析）
  → 成功後記錄來源為「PNG W__」

③ 最後備案：讀 GitHub daily log 的 txt（①② 均失敗時）
  路徑：/Users/tittan/program/github/trading/xauusd/daily_log/weekly_report_*.txt
  找最新的 weekly_report_*.txt，用 Read 工具直接讀取
  → 成功後記錄來源為「txt W__」
...
1. 呼叫 mcp__computer-use__request_access 取得 Chrome 操作權限
   （若 Step 2 走①路徑已執行還原步驟，Chrome 前景應已是 TradingView，直接截圖）
...
⚠ 不使用 Yahoo Finance / web_fetch / CSV 取代截圖

備用方式（僅當 computer use 完全無法操作時）：
讀取 xauusd/csv/ 或 googledrive/XAUUSD/weekly report/csv/ 最新 30m CSV，並在輸出中標注「使用 CSV，非即時截圖」
```
