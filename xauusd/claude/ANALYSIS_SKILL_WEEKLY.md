...
> 💡 提醒（20260706c，配合 ANALYSIS_SKILL.md「劇本狀態追蹤」）：三種劇本描述時，
> 若能順手給出概略「目標區間」（例如4300-4390）或明確的「關鍵位清單」，會讓每日
> 「請分析」判斷劇本是否已經走完（completed）更精準；但**不強制**——劇本本質是
> 大方向敘述，沒有具體數字時純文字描述即可，不需要為了湊格式硬造不存在的精確度。
