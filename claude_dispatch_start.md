...
6. 讀取指令手冊：
   https://raw.githubusercontent.com/tittanliao/trading/main/claude_dispatch_recovery.md
...
| 指令 | 說明 |
|------|------|
| `請分析` | 截取 TradingView XAUUSD 30m 截圖 → 分析 → 更新 xauusd_latest.json → 提醒 git commit/push（當天第一筆） |
...
- **Git commit/push**：沙盒無 .git 寫入權限，分析後提醒用戶在 Terminal 手動執行：
  ```bash
  cd ~/program/github/trading && git add xauusd/daily_log/ && git commit -m "daily analysis: $(date +%Y-%m-%d)" && git push
  ```
